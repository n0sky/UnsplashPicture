# -*- coding: utf-8 -*-
import scrapy, sqlite3, threading, json

file_dir = "./database/link.db"
...
class PicSpider(scrapy.Spider):
    ...
    def start_requests(self):  # Scrapy启动时调用该函数
        ...
        begin, page = 1, 3  # 设定开始页面以及要爬的页面数
        conn = sqlite3.connect(file_dir)  # 连接数据库
...
def createDB():  # 创建数据库
    conn = sqlite3.connect(file_dir)  # Sqlite是一个轻量数据库，不占端口，够用
